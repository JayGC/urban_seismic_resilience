...
def start():
    global global_ctrl
    config_path = os.path.join('configs', 'trap_scenario.yaml')
    if os.path.exists(config_path):
        with open(config_path) as f:
            config = yaml.safe_load(f)
        # RQ4 greedy mode: set RQ4_MODE=decentralized to visualize decentralized (greedy) scenario
        if os.environ.get('RQ4_MODE') == 'decentralized':
            config = config.copy()
            config['controller_mode'] = 'decentralized'
